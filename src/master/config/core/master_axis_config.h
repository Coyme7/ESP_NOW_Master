...
static constexpr MasterAxisInputConfig kMasterXAxisInput = {
    180.0f, // X 机械中位绝对角度，单位 deg。
    -1,     // X 输入坐标方向符号。
    0,      // X 协议坐标中心死区，单位 counts。
};

// Y 轴输入标定：编码器中位、输入方向和协议死区。
static constexpr MasterAxisInputConfig kMasterYAxisInput = {
    180.0f, // Y 机械中位绝对角度，单位 deg。
    -1,     // Y 输入坐标方向符号。
    0,      // Y 协议坐标中心死区，单位 counts。
};
...
#else
    0.25f, // X 常规目标电流限幅，单位 A。
    30.0f, // X 常规目标电流爬升斜率，单位 A/s。
#endif
#else
    0.020f, // X 电压模式等效目标限幅，单位 A。
    2.4f,   // X 电压模式等效爬升斜率，单位 A/s。
#endif
    500.0f, // X 目标电流释放斜率，单位 A/s。
};
